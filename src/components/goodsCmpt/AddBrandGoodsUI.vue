<template>
  <div class="popup-AddBrandGoodsUI" v-on:click.self="showAddUI(false)">
    <div>
      <div class="header">
        <img :src="icon_closeX" v-on:click="showAddUI(false)" />
        <span>新增商品品牌</span>
      </div>
...
const { addGoodsBrandApi } = apptstore;
...
function submitBtn() {
  ...
  /**新增 */
  addGoodsBrandApi(apiData).then((res: any) => {
    if (res.state == 1) {
