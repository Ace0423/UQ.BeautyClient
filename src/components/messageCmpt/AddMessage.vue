...
onMounted(() => {
  if (props.selectMessageItem.item == "") {
    ...
    messageData.mSendDateD =
      props.selectMessageItem.item.mSendDateD == "0001-01-01"
        ? ""
        : props.selectMessageItem.item.mSendDateD;
    ...
    if (
      (messageData.mSendTimer == 2 && messageData.mSendTimeing == 2) ||
      (messageData.mSendTimer == 2 && messageData.mSendTimeing == 3)
    ) {
...
const handSubmit = () => {
  switch (messageData.mSendTimer) {
    case 2:
      ...
      if (messageData.mSendTimeing == 2) {
        ...
        messageData.mSendDateD =
          yy + "-" + add0(mm) + "-" + add0(messageData.mSendDateD);
      } else if (messageData.mSendTimeing == 3) {
        messageData.mSendDateD =
          yy + "-" + add0(mSend.value) + "-" + add0(dSend.value);
      }
      break;
    default:
  ...
  messageData.mMapCouponCards =
    mMapCouponCards.value.length == 0 ? [] : [mMapCouponCards.value[0].ccId];
  ...
  <div class="popup-mask" v-on:click.self="handAddMessageView()">
    ...
    <div class="popup-content">
      <div class="top-content">
        ...
        <span>{{ title }}</span>
        <button class="otherpay-btn" v-on:click="handSubmit()">
          {{ submitNameBtn }}
        </button>
      </div>
      <div class="news-wrap" v-if="props.selectMessageItem.type == 1">
        <div class="message-info">
          ...
          <div class="auto">
            ...
            <h2 v-if="props.selectMessageItem.auto == 1">
              開始建立您的自動化訊息
            </h2>
            <p>請輸入訊息主題</p>
        ...
        <div class="example-img">
          ...
          <div class="example-bg">
            <div>
              <div>
                <div class="top-block"><img :src="Icon" /></div>
                <div class="now-time">
                  <p>{{ nowTime }}</p>
                </div>
                <div class="example-content">
                  <div class="msg-content">
                    <p class="p-content">{{ messageData.mContext }}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="hint-content">
            <p>依據簡訊編碼標準，此訊息內容共計2點</p>
          </div>
          <div class="test-content">
            <input placeholder="輸入手機號碼(例:0912345678)" /><button>發送測試訊息</button>
            <p>發送測試訊息將酌收簡訊費用</p>
          </div>
        </div>
      </div>
      <div class="line-wrap" v-if="props.selectMessageItem.type == 0">
        ...
        <div class="example-img">
          ...
          <div class="hint-content">
            <p>
              發送訊息機使用您的LINE OA
              訊息量LINE圖片支援最大高比例為1:3實際訊息圖片以LINE訊息為主
            </p>
          </div>
        </div>
      </div>
