<template>
    <div class="popup-detailService" v-on:click.self="showManagerUIFn(0)">
        <div class="popup-content">
            <div class="header-content">
                <span>服務人員</span>
                <img :src="icon_closeX" v-on:click="showManagerUIFn(0)" />
            </div>
            <div class="main-content">
                <input placeholder="搜尋" v-model="formInputRef.search" />
                <div class="list-content">
                    <div v-for="item in filterManagerData" :key="item">
                        <label class="label-item" :value="item">
                            <input class="input-item" type="radio" :key="item" :value="item" :id="'RdManagerUI_' + item"
                                v-model="formInputRef.managerItem" @click="clickItem(item, item.managerId)" />
                            <!-- <label :for="'RdManagerUI_' + item.managerId"></label> -->
...
let { managerList } = storeToRefs(store);
let { getManagerListApi } = store;

const props = defineProps<{
    showManagerUIFn: Function;
    getDataFn: Function;
    selData: any;
}>();
...
let filterManagerData: any = computed(() =>
    managerList.value.filter(getManagerFn)
);
function getManagerFn(data: any) {
    return (
        !data.userLock &&
